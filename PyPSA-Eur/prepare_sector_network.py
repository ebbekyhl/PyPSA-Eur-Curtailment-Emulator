...
def add_emission_price(n):
    ...
    n.links.loc[bus2_emittors.index,"marginal_cost"] = new_marginal_price_bus2_emittors
    ########################################################################
    return n
...
def add_LDES(n,costs):
    logger.info("Add LDES")

    nodes = n.buses.query("carrier == 'AC'").index

    n.add("Carrier", "LDES")

    n.madd("Bus", nodes + " LDES", location=nodes, carrier="LDES", unit="MWh_LHV")
    ...
    n.madd(
        ...
        bus0=nodes,
        p_nom_extendable=True,
        carrier="LDES charger",
        efficiency=costs.at["LDES charger", "efficiency"],
        capital_cost=costs.at["LDES charger", "fixed"],
        lifetime=costs.at["LDES charger", "lifetime"],
    )

    n.madd(
        "Link",
        nodes + " LDES discharger",
        bus0=nodes + " LDES",
        bus1=nodes,
        p_nom_extendable=True,
        carrier="LDES discharger",
        efficiency=costs.at["LDES discharger", "efficiency"],
        capital_cost=costs.at["LDES discharger", "fixed"]*costs.at["LDES discharger", "efficiency"],  # NB: fixed cost is per MWel
        lifetime=costs.at["LDES discharger", "lifetime"],
    )
    ...
    n.madd(
        ...
        e_nom_extendable=True,
        e_cyclic=True,
        carrier="LDES storage",
        capital_cost=costs.at["LDES storage", "fixed"],
    )

def add_h2_network(n,costs):
...
def add_biomass(n, costs, opts):
    ...
    if options["biomass_boiler"] and "H" in opts:
...
def maybe_adjust_costs_and_potentials(n, opts):
    for o in opts:
        if o == 'greenfield':
            ...
            n.mremove("StorageUnit", storage_units.index)
            ror_generators = n.generators.query("carrier == 'ror'")
            n.mremove("Generator", ror_generators.index)
            logger.info("removed hydro (reservoir + pumped hydro storage + run-of-river)")
            continue
        
        elif o == "LDES":
            add_LDES(n,costs)
            logger.info("added LDES")
            continue

        elif o == "noSDES":
            battery_stores = n.stores[n.stores.carrier.str.contains("battery")]
            n.mremove("Store", battery_stores.index)
            logger.info("removed batteries")
            continue
        
        elif o == "noH2fuelcell":
            fuel_cells = n.links.query("carrier == 'H2 Fuel Cell'")
            n.mremove("Link", fuel_cells.index)
            logger.info("removed H2 Fuel Cells")
            continue

        elif o == "H2network":
            add_h2_network(n,costs)
            logger.info("added option of H2 pipeline")
            continue

        elif o == "lifetimerenewables":
            add_lifetime_wind_solar(n, costs)
            logger.info("added lifetime of wind and solar generators")
            continue
        ...
        # if oo[0] == 'copperplate':
            # Then aggregate nodes into single region
...
if __name__ == "__main__":
    ...
    if "B" in opts:
        add_biomass(n, costs, opts)

    if options["ammonia"]:
        add_ammonia(n, costs)

    if "I" in opts:
        add_industry(n, costs, opts)
