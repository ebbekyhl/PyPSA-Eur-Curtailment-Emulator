...
def calculate_theoretical_curtailment(scen, technologies, model_years):
...
def calculate_theoretical_curtailment_new(scen, technologies, model_years):
    ...
    for y in model_years:
        ...
        for i,j in gamma_wind.index:
            wind_th_curtailment[i,j] = gamma_wind.loc[i,j]*df_curtailment_y.loc["wind_curtailment" + str(i)].lvl  

        for i,j in gamma_solar.index:
            solar_th_curtailment[i,j] = gamma_solar.loc[i,j]*df_curtailment_y.loc["solar_curtailment" + str(i)].lvl
...
def calculate_tech_contribution(scen,renewable, model_years, bins):
    tech_contribution = {}
    for i in bins:
        ...
        act = scen.var("ACT", {"technology": tec_act, 
                            "year_act": model_years,
                            "node_loc": "R11_WEU"}).groupby(["technology","year_act"]).sum().lvl
...
def calculate_total_generation(scen, model_years):
...
def plot_generation_and_capacity(scen, technologies, model_years, bins, formulation="old", plot=True):
    ...
    if formulation == "old":
        theor_wind_curtailment_sum, theor_solar_curtailment_sum, wind_resources, solar_resources = calculate_theoretical_curtailment(scen, technologies, model_years)
    else:
        theor_wind_curtailment_sum, theor_solar_curtailment_sum, wind_resources, solar_resources = calculate_theoretical_curtailment_new(scen, technologies, model_years)

    tech_contribution_wind_sum, tech_cap_w, tech_act_w = calculate_tech_contribution(scen,relation1, model_years, bins)
    tech_contribution_solar_sum, tech_cap_s, tech_act_s = calculate_tech_contribution(scen,relation2, model_years, bins)
    
    df_generation, df_capacity, colors_dict = calculate_total_generation(scen, model_years)
    ...
    # tech_cap_w.rename(columns={"h2_elec":"h2","stor_ppl":"stor"},inplace=True)
