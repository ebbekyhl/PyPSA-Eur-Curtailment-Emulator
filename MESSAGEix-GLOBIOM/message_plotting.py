...
def plot_Behnam_script(scenarios,region_name,path_result,plot_curtailment = True):
    # %% Running the setup on scenario results

    # Loading a scenario
    #mp = ix.Platform(name="ixmp_dev", jvmargs=["-Xms800m", "-Xmx8g"])
    ...
    if plot_curtailment:
        ...
        #     "Coal w CCS": "grey",
        #     "Biomass": '#baa741',
        #     "Biomass w CCS": '#baa741',
        #     "Solar PV": "#f9d002",
        #     "Solar PV (th-curt)1": "yellow",
        #     "Solar PV (th-curt)2": "gold",
        #     "Solar PV (th-curt)3": "goldenrod",
        #     "Solar PV curtailed": "orange",
        #     "Wind": "#235ebc",
        #     "Wind (th-curt)1": "mediumturquoise",
        #     "Wind (th-curt)2": "cadetblue",
        #     "Wind (th-curt)3": "darkcyan",
        #     "Wind curtailed": "blue",
        #     "Nuclear": '#ff8c00',
        #     "Oil": '#c9c9c9',
        #     "Storage": '#ace37f',
        ...
        # # Basis of the results ("value" or "share")
        # results_based = "value"

        # # Loop over relations for post-processing and plotting
        # curt_tot = {}
        # for scen in scenarios.keys():
        #     curt_tot[(scen, "Wind")] = curt_tot[(scen, "Solar PV")] = pd.DataFrame()

        #for num, (scen, r) in enumerate(results.keys()):
        #     if results_based == "share":
        #         droping = ["value"]
        #     else:
        #         droping = ["share"]
        ...
        #         results[(scen, r)]
        #         .reset_index()
        #         .rename({"technology": "variable", "year_act": "year"}, axis=1)
        #         .drop(droping, axis=1)
        #     )
        #     val = val.rename({"share": "value"}, axis=1)
        #     val["model"] = sc.model
        #     val["scenario"] = sc.scenario
        #     val["unit"] = "GW"
        #     val["region"] = region_name[0]

        #     # Renaming technologies to legend labels
            # t_list = []
            # ren = {}
            # for x, y in rename.items():
            #     for z in y:
            #         ren[z] = x
            #         t_list = t_list + [z]
            # missing_label = [x for x in set(val["variable"]) if x not in t_list]
            # if missing_label != []:
            #     print(
            #         ">>>WARNING<<<: There is no label for the following technologies: {}".format(
            #             missing_label
            #         )
            #     )
            # val["variable"] = val["variable"].replace(ren)
            # val = val.groupby(idx + ["year"]).sum().reset_index()

        #     # Calculating theoretical curtailment needs
        #     if "curtailment" in r:

        #         # Load has (-) sign and generation (+)
        #         varbl = "Wind" if "wind" in r else "Solar PV"
        #         val.loc[val["variable"] == varbl, "value"] += val.loc[
        #             val["variable"] == "Load", "value"
        #         ].values
        #         val.loc[(val["variable"] == varbl) & (val["value"] < 0), "value"] = 0
        #         val = val.loc[~val["variable"].isin(["Load"])].copy()

        #         # Multiplying all values (all techs) by the input eff. of
        #         # curtailment technologies
        #         val["value"] *= inp_curt[r]

        #         # Renaming to theoretical curtailment
        #         val["variable"] = val["variable"].replace(
        #             {varbl: varbl + " (th-curt)" + r[-1]}
        #         )

        #         # Adding to total curtailment
        #         curt_tot[(scen, varbl)] = (
        #             pd.concat([val, curt_tot[(scen, varbl)]])
        #             .groupby(idx + ["year"])
        #             .sum()
        #             .reset_index()
        #         )

        #     # Excluding some variables from reserve margin
        #     if "Reserve margin" in r:
        #         val = val.loc[~val["variable"].isin(["Load"])].copy()

        #     # Pivot table
        #     val = pd.pivot_table(val, index=idx, columns="year", values="value").reset_index()

        #     # Finding order
        #     od = [x for x in order if x in set(val["variable"])]
        #     val = val.set_index(["variable"]).loc[od, :].reset_index()

        #     # Saving the results for plotting
        #     figures[(scen, r)] = val

        # # Adding total curtailments to the figure lists
        # for scen, v in product(scenarios.keys(), ["Wind", "Solar PV"]):
        #     curt_tot[(scen, v)] = pd.pivot_table(
        #         curt_tot[(scen, v)], index=idx, columns="year", values="value"
        #     ).reset_index()
        #     figures[(scen, v + " Curtailment (GWa)")] = curt_tot[(scen, v)]

        # #% Visualization (one figure per panel)
        # using_pyam = False
        # for scen, r in figures.keys():
        #     # Unit for figure titles
        #     r = r.split("GW")[0] + "%)" if results_based == "share" else r

        #     # For curtailment only visualize the total Curtailment figures
        #     if "curtailment" in r:
        #         continue
        #     val = figures[(scen, r)]
        #     fig = plt.figure(num, figsize=(8, 4))
        #     ax = fig.add_subplot(111)
        #     tit = r + ", " + region_name[0] + ", " + scen

        #     if using_pyam:
        #         # pyam package
        #         df = pyam.IamDataFrame(val)
        #         # df = df.filter(year=[2030, 2050, 2100]).timeseries()
        #         df.plot.bar(
        #             stacked=True,
        #             title=tit,
        #             bars_order=order,
        ...
        #         )
        #     else:
        #         df = val[model_years + ["variable"]].set_index("variable").T
        #         #my_colors = [color_map.get(x) for x in df.columns]
        #         df.plot(ax=ax, kind="bar", stacked=True, title=tit, 
        #                 color=[color_map[i] for i in df.columns])
        ...
        #         # title=legend_title, facecolor='white', ncol=legend_columns,
        #         fontsize=9,
        #         framealpha=1,
        #     ).get_frame()
        ...
        # #% Visualization with share (2 figures per panel)
        # plt.close("all")
        # using_pyam = False
        # figures_share = {}

        # with PdfPages(path_result + "\\vre-integration_" + scen + ".pdf") as pdf:
        #     for scen, r in figures.keys():
        #         # For curtailment only visualize the total Curtailment figures
        #         if "curtailment" in r:
        #             continue
        ...
        #         fig.suptitle(r.split(" (GW")[0] + ", " + region_name[0] + ", " + scen, fontweight="bold")

        #         for num, ax in enumerate(axes.reshape(-1)):
        #             # Title
        #             tit = "(" + r.split("(")[1] if num == 0 else "(%)"
        ...
        #             # Share
        #             if num == 1:
        #                 val = val.set_index(idx)
        #                 val[val[model_years] > 0] /= val[val[model_years] > 0].sum()
        #                 val[val[model_years] <= 0] /= -val[val[model_years] <= 0].sum()
        #                 val = val.reset_index()
        #                 figures_share[(scen, r.split("(")[0] + "(%)")] = val
